...
\begin{itemize}
    ...
    \item \textbf{Tkinter \cite{tkinter}} : pour l'interface utilisateur.
    \item \textbf{Plotly \cite{ploty}} : pour la visualisation du circuit optimisé sur une carte.
...
Le problème de décision associé au TSP est \cite{np_completude}
\textbf{NP-Complet}. Il peut notamment être réduit à partir du problème du cycle Hamiltonien \cite{cycle_hamiltonien}, un des 21 problèmes NP-Complets de Karp.
Ainsi, trouver la solution exacte devient rapidement intraitable lorsque le nombre de villes augmente, rendant nécessaire l'utilisation d'algorithmes d'approximation tels que les algorithmes génétiques.
...
\begin{itemize}
    \item \textbf{Fonction de Sélection :} \emph{Tournoi \cite{selection_tournoi}}. On choisit aléatoirement $k$ individus et l'on sélectionne les deux meilleurs selon la fonction \textit{fitness}.
...
La mutation est une \textbf{swap mutation} avec une probabilité $p = 2\,\%$. Si une valeur $q$, générée aléatoirement est inférieur à $p$, deux villes sont sélectionnées aléatoirement et échangées dans l’individu.

\subsection{Stratégie de remplacement}
La population suivante est constituée uniquement des enfants générés par celle qui l'a précédée : c'est un \textbf{remplacement générationnel \cite{remplacement_gen}}.
...
\begin{itemize}
    \item \textbf{Tkinter \cite{tkinter}} pour l'interface utilisateur et l’affichage de la feuille de route.
    \item \textbf{Plotly \cite{ploty}} pour tracer le circuit optimisé sur une carte.
\end{itemize}
...
La taille des populations à chaque génération était de 100 individus et l'algorithme a été exécuté sur 500 générations. Nous avons comparé les résultats obtenus par notre algorithme génétique avec les solutions exactes obtenues par une approche de brute-force pour les instances de taille réduite ($n \leq 10$ villes).

Pour $n < 10$, l'algorithme trouve toujours la solution optimale ; pour $n = 10$, il trouve une solution à 1,5\,\% de l'optimal. Pour $n > 10$, le brute-force est très lent pour le calcul de l'optimum. Nous pensons que les résultats de ces tests attestent du bon fonctionnement de notre algorithme pour un nombre quelconque de ville.
...
\begin{itemize}
    ...
    \item Utiliser la bibliothèque \texttt{tsplib} \cite{tsblib} pour comparer les performances de notre algorithme génétique avec d'autres approches publiées.
